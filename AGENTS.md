...
> Five-line orientation for any AI coding agent (Claude Code, Codex, Cursor, Cline, Aider, etc.) entering this repo. Generated by kernel `ingest-project.py` on 2026-05-21T23:49:12.308814+00:00; canonical methodology lives in the cowork kernel.
...
Why: kernel canonical state (`_cowork/PROJECT-MAP.json`, `plugin/tools/`, `plugin/commands/`, the kernel-side `_cowork/dashboard-delaney.json`) lives in the kernel folder. Every preset reads PROJECT-MAP.json + bare-name tool resolution from kernel during its Step-1 handshake. Single-mount Cowork tasks fail with `[BLOCK] kernel-mount-not-reachable` at handshake time.
...
*Generated by kernel KM316 ingest-project.py. Project prefix: DM. Mount: C:\CC\delaney.*

<!-- eof: agents-md-delaney-v1 -->
